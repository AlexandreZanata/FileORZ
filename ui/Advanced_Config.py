...
def open_advanced_config_window(parent):
    ...
    window.title("Configurações Avançadas - FileORZ")
    ...
    header_title = customtkinter.CTkLabel(
        header_inner, 
        text="Organização Avançada de Documentos",
    ...
    checkbox_text = "Modo Avançado Ativado" if advanced_var.get() else "Modo Avançado Desativado"
    ...
    def refresh_keywords():
        ...
        for category, words in keywords_data.items():
            ...
            def save_group(old_cat=category, n_entry=name_entry, w_entry=words_entry):
                ...
                if not new_cat:
                    n_entry.configure(border_width=1, border_color=COLORS["accent_danger"])
                    return
